...
超级管理员对用户组进行管理，设置组管理员；组管理员负责审核用户的加入申请，加入后该用户即为该用户组的报单员；

用户组向本系统上报安装服务订单，代收安装费用，并定期与本系统进行结算，结算价格在用户组中设置
...
- 本系统与分组结算：
  ...
  - 本系统管理员向分组要求结算一定范围内的安装费用（不经过本系统），创建结算单据
  - 本系统管理员确认收款之后，对结算单据确认收款
