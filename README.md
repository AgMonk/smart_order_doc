...
**派单员添加为某一地区的成员，才可以与该地区的订单进行互动**  

#### 安装员管理

由派单员对这一地区的安装员进行管理，审核申请，移除成员
...
- 由超管将某一用户添加为某一用户组成员后，将其设置为组管理员；**添加管理员不要走申请流程**
...
- 与分组结算：
  - 派单员向分组要求结算一定时间范围内的安装费用（不经过本系统），分组转账完成后通知派单员
  - 派单员在本系统查询该时间范围内的订单，确认与转账金额一致后结算该范围内的订单（将这些订单的"订单支付状态"设置为已支付(其他)）
- 与安装员结算：
  - 派单员在本系统查询该时间范围内的订单
  - 派单员指定订单价格的百分比，或直接指定单价，对订单执行结算操作
  - 派单员向安装员支付 （订单价格 * 百分比 ） 或 （订单数量 * 单价） 的费用（不经过本系统）
